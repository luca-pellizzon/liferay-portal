...
public interface ObjectEntryCMSBulkActionResource {
	...
	public void postObjectEntryCMSBulkActionBatch(
			String bulkActionName, String[] entryIds, String search,
			String filterString, String callbackURL, Object object)
		throws Exception;
	...
	public static class ObjectEntryCMSBulkActionResourceImpl
		...
		public ObjectEntryCMSBulkAction postObjectEntryCMSBulkAction(
			...
			HttpInvoker.HttpResponse httpResponse =
				postObjectEntryCMSBulkActionHttpResponse(
					bulkActionName, entryIds, search, filterString);
			...
			try {
				return ObjectEntryCMSBulkActionSerDes.toDTO(content);
			}
			catch (Exception e) {
				_logger.log(
					Level.WARNING,
			...
			if (entryIds != null) {
				for (int i = 0; i < entryIds.length; i++) {
					httpInvoker.parameter(
						"entryIds", String.valueOf(entryIds[i]));
				}
			}
		...
		public void postObjectEntryCMSBulkActionBatch(
			...
			HttpInvoker.HttpResponse httpResponse =
				postObjectEntryCMSBulkActionBatchHttpResponse(
					bulkActionName, entryIds, search, filterString, callbackURL,
					object);
